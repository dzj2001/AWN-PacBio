...
针对小麦基因组庞大、重复序列多、亚基因组间信号混杂等挑战，AWN 对 Cue 原有的四阶堆叠沙漏网络进行了三项核心改进，提出 RF-HGNet网络：
...
AWN 支持检测以下 SV 类型：DEL、DUP、INV、INVDEL、INVDUP，同时兼容 PacBio 平台的 CCS（HiFi 高保真数据）与 CLR（连续长读长数据）两种数据格式。
